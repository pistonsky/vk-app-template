...
class RateLimiter extends ActionFilter
{
    ...
    public function beforeAction($action)
    {
        ...
        if ($user instanceof RateLimitInterface) {
            Yii::warning('Check rate limit', __METHOD__);
            $this->checkRateLimit(
                $user,
            ...
            Yii::warning('Rate limit skipped: "user" does not implement RateLimitInterface.', __METHOD__);
        } else {
            Yii::warning('Rate limit skipped: user not logged in.', __METHOD__);
        }
        return true;
    }
